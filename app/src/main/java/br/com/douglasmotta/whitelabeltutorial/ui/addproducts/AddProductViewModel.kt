...
class AddProductViewModel @Inject constructor(
    private val createdProductsUseCase: CreateProductUseCase
) : ViewModel() {
    ...
    fun createProducts(description: String, price: String, imageUri: Uri?) =
        viewModelScope.launch {
        isFormValid = true

        _imageUriErrorResId.value = getDrawableResIdIfNull(imageUri)
        _descriptionFieldErrorResId.value = getErrorStringResIdIsEmpty(description)
        _priceFieldErrorResId.value = getErrorStringResIdIsEmpty(price)

        if(isFormValid) {
            try {
                val product = createdProductsUseCase(description, price.fromCurrency(), imageUri!!)
    ...
    private fun getErrorStringResIdIsEmpty(value: String): Int? {
